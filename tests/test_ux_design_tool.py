...
class TestUXDesignTool:
    ...
    def test_validate_wcag_large_text_threshold(self):
        # Texto grande necesita solo 3.0:1
        result = validate_wcag_contrast("#666666", "#999999", is_large_text=True)
        data = json.loads(result)
        # Este contraste suele estar cerca del límite, verificamos que la lógica de is_large_text funcione
    ...
    def test_audit_performance_budget_high_risk(self):
        ...
        data = json.loads(result)
        assert data["evaluation"]["inp_risk"] == "ALTO"
        assert data["evaluation"]["lcp_risk"] == "ALTO"
    ...
    def test_audit_performance_budget_low_risk(self):
        ...
        data = json.loads(result)
        assert data["evaluation"]["inp_risk"] == "BAJO"
        assert data["evaluation"]["lcp_risk"] == "OPTIMO"
